...
local function tostring_can_be_table(maybe_tbl, o, c, sep, seen)
  -- if Common.Table.is(maybe_tbl) and seen[maybe_tbl] ~= nil then
  --   return 'CYCLE'
  -- elseif Common.Table.is(maybe_tbl) then
  if Common.Table.is(maybe_tbl) then
    seen[maybe_tbl] = true
    return Common.Table.tostring(maybe_tbl, o, c, sep, seen)
  end
...
function Common.Table.tostring(tbl, o, c, sep, seen)
  seen = seen or {}
  ...
  for k, v in pairs(tbl) do
    local key_str = tostring_can_be_table(k, o, c, sep, seen)
    local val_str = tostring_can_be_table(v, o, c, sep, seen)
    ...
    if arr_str ~= nil and tbl[j] ~= nil then
      nxt = (j == 1 and '' or sep) .. tostring_can_be_table(v, o, c, sep, seen)
      arr_str = arr_str .. nxt
      j = j + 1
  ...
  if Common.String.nil_or_empty(arr_str) and Common.String.nil_or_empty(str) then
    return o .. c
  end

  return Common.String.fmt('%s %s %s', o, arr_str or str, c)
end
