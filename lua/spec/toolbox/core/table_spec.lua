...
describe('Table', function()
  ...
  describe('.keys(tbl, transform)', function()
    ...
    it("should transform a dict-like table's keys", function()
      assert.same(
        Table.keys({ a = 'z', b = 'y', c = 'x' }, transform),
        { 'a-z', 'b-y', 'c-x' }
      )
    end)
  end)
  ...
  describe('.values(tbl, transform)', function()
    ...
    it("should transform an array-like table's values", function()
      assert.same(
        Table.values({ 'a', 'b', 'c', 'd' }, transform),
        { 'a-1', 'b-2', 'c-3', 'd-4' }
      )
      assert.same(Table.values({}), {})
    end)
    it("should transform a dict-like table's values", function()
      assert.True(Common.Array.equals(
        Table.values({ a = 'z', b = 'y', c = 'x' }, transform),
        { 'z-a', 'y-b', 'x-c' }
      ))
    end)
  end)
  ...
  describe('.pack(...)', function()
    it('should return a table filled with the provided arguments', function()
      assert.same(
        Table.pack(1, 2, 3, 'a', true, nil),
        { 1, 2, 3, 'a', true }
      )
      assert.same(
        Table.pack(1, 2, 3, 'a', true, { 'a', 'b' }),
        { 1, 2, 3, 'a', true, { 'a', 'b' }}
      )
    end)
  ...
  describe('.get_only(tbl, strict)', function()
    ...
    describe('strict is true', function()
      ...
      it('should raise an error if the tbl is empty', function()
        assert.has_error(function() Table.get_only({}) end)
      end)
      it('should raise an error if there is more than one key/value pair in tbl', function()
        assert.has_error(function() Table.get_only({a = 1, b = 2}) end)
      end)
    end)
  end)
  ...
  describe('.map_items(tbl, xfms)', function()
    describe('dict-like tables', function()
      ...
      local keys = function(k, v, _) return k .. tostring(v) end
      local vals = function(v, _, _) return v + 1 end

      it('should map keys', function()
        assert.same(
          Table.map_items(tbl, { keys = keys }),
          { a1 = 1, b2 = 2, c3 = 3 }
        )
      end)
      it('should map values', function()
        assert.same(
          Table.map_items(tbl, { vals = vals }),
          { a = 2, b = 3, c = 4 }
        )
      end)
      it('should map keys and values', function()
        assert.same(
          Table.map_items(tbl, { keys = keys, vals = vals }),
          { a1 = 2, b2 = 3, c3 = 4 }
        )
      end)
    end)
    ...
    describe('array-like tables', function()
      ...
      local arr = { 1, 2, 3 }
      local keys = function(k, _, i) return k + i end
      local vals = function(v, _, i) return v + i end

      it('should map keys', function()
        assert.same(
          Table.map_items(arr, { keys = keys }),
          { [2] = 1, [4] = 2, [6] = 3 }
        )
      end)
      it('should map values', function()
        assert.same(
          Table.map_items(arr, { vals = vals }),
          { 2, 4, 6 }
        )
      end)
      it('should map keys and values', function()
        assert.same(
          Table.map_items(arr, { keys = keys, vals = vals }),
          { [2] = 2, [4] = 4, [6] = 6 }
        )
      end)
  end)
  ...
  describe('.to_dict(arr, xfms)', function()
    ...
    local arr = { 'a', 'b', 'c' }
    local mapper = function(e, i) return e .. i end

    it('should use the key mapper', function()
      assert.same(
        Table.to_dict(arr, { keys = mapper }),
        { a1 = 'a', b2 = 'b', c3 = 'c' }
      )
    end)
    it('should use the value mapper', function()
      assert.same(
        Table.to_dict(arr, { vals = mapper }),
        { a = 'a1', b = 'b2', c = 'c3' }
      )
    end)
    it('should use the key and value mappers', function()
      assert.same(
        Table.to_dict(arr, { keys = mapper,  vals = mapper }),
        { a1 = 'a1', b2 = 'b2', c3 = 'c3' }
      )
    end)
    it('should use a single mapper if provided', function()
      assert.same(
        Table.to_dict(arr, function(e, i) return e .. i, e .. i end),
        { a1 = 'a1', b2 = 'b2', c3 = 'c3' }
      )
    end)
  ...
  describe('.combine_many(tbls)', function()
    ...
    it('should prioritize (avoid overwriting) values further to the right (later in input array)', function()
      ...
      assert.same(
        Table.combine_many({ ichi, ni, san }),
        { a = 1, b = 4, c = 3, d = 5 }
      )
    end)
    it('should work w/ nested tables', function()
  ...
  describe('.reverse_items(tbl, fail_on_dup)', function()
    it('should reverse keys/values in tbl', function()
      assert.same(
        Table.reverse_items({ a = 'z', b = 'y', c = 'x' }),
        { z = 'a', y = 'b', x = 'c' }
      )
    end)
    it('should work if the resulting table is array-like', function()
      assert.same(
        Table.reverse_items({ a = 1, b = 2, c = 3 }),
        { 'a', 'b', 'c' }
      )
    end)
    it('should work if the resulting table is array-like', function()
    ...
    it('should work w/ array-like tables', function()
      assert.same(
        Table.reverse_items({ 'a', 'b', 'c' }),
        { a = 1, b = 2, c = 3 }
      )
    end)
    it('should work w/ duplicate keys', function()
      assert.same(
        Table.reverse_items({ 'a', 'b', 'c', 'c' }),
        { a = 1, b = 2, c = 4 }
      )
    end)
